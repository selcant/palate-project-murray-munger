...
load("/projects/munger-lab/projects/palate-project-murray-munger/read_mapping/palate_gbrs_emase_counts_v4.RData") # raw_expr_mat, expr_mat_do, expr_mat_nondo , new genome assembly
...
Next, I picked strategy #2 from the first comparison and analyzed the CAST data and the B6/CAST data combined. Here the main point of comparison is to see if analyzing the datasets separately is better than analyzing them together.  
...
```{r}
#| label: module_genes_table_b6_full
#| message: false
#| warning: false


net_b6_full$genes_and_modules |> 
  left_join( all_genes_palate |> 
...
plot_eigengene_by_module_genes <- function(mod_name, module_eigengenes_long, expr, coldata,module_genes){
    ...
    gene_plot <- pheatmap(mat = mat,
         ...
         annotation_col = coldata[,c("AP_domain","LB_stage","Palate_ori")],
...
#| fig-width: 10
#| fig-height: 8
corrplot(corr = cormat_full[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F],
         p.mat = as.matrix(pmat_full[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]),
         sig.level = 0.05,
         insig = "label_sig", 
         pch.cex = 1.3, 
         pch.col = "red",
...
#| warning: false
#| message: false
cormat_full[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]|> 
  as_tibble(rownames = "module") |> 
  pivot_longer(2:9, names_to = "effect", values_to = "Correlation") |> 
  full_join(
    pmat_full[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F] |> 
      as_tibble(rownames = "module") |> 
...
```{r}
#| label: module_genes_table_b6_top5
#| message: false
#| warning: false


net_b6_top5$genes_and_modules |> 
  left_join( all_genes_palate |> 
               select( Genes = gene_id, symbol, chromosome)
            ) |> 
  group_by(Modules) |> 
...
#| fig-width: 10
#| fig-height: 6
corrplot(corr = cormat_top5[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F],
         p.mat = as.matrix(pmat_top5[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]),
         sig.level = 0.05,
         insig = "label_sig", 
         pch.cex = 1.3, 
         pch.col = "red",
...
#| warning: false
#| message: false
cormat_top5[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]|> 
  as_tibble(rownames = "module") |> 
  pivot_longer(2:9, names_to = "effect", values_to = "Correlation") |> 
  full_join(
    pmat_top5[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F] |> 
      as_tibble(rownames = "module") |> 
      pivot_longer(2:9, names_to = "effect", values_to = "Adjusted p-value")
  ) |> 
  filter(`Adjusted p-value` < 0.05) |> 
...
```{r}
#| label: module_genes_table_b6_top25
#| message: false
#| warning: false


net_b6_top25$genes_and_modules |> 
  left_join( all_genes_palate |> 
               select( Genes = gene_id, symbol, chromosome)
            ) |> 
  group_by(Modules) |> 
...
#| fig-width: 10
#| fig-height: 6
corrplot(corr = cormat_top25[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F],
         p.mat = as.matrix(pmat_top25[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]),
         sig.level = 0.05,
         insig = "label_sig", 
         pch.cex = 1.3, 
         pch.col = "red",
...
#| warning: false
#| message: false
cormat_top25[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]|> 
  as_tibble(rownames = "module") |> 
  pivot_longer(2:9, names_to = "effect", values_to = "Correlation") |> 
  full_join(
    pmat_top25[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F] |> 
      as_tibble(rownames = "module") |> 
      pivot_longer(2:9, names_to = "effect", values_to = "Adjusted p-value")
  ) |> 
  filter(`Adjusted p-value` < 0.05) |> 
...
```{r}
#| label: module_genes_table_cast_top5
#| message: false
#| warning: false
...
#| fig-width: 10
#| fig-height: 6
corrplot(corr = cormat[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F],
         p.mat = as.matrix(pmat[,c("M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]),
         sig.level = 0.05,
         insig = "label_sig", 
         pch.cex = 1.3, 
         pch.col = "red",
...
b6_and_cast <- c(b6=b6_expr_top5[intersect(rownames(b6_expr_top5), rownames(cast_expr)),,drop=F], 
                              cast = cast_expr[intersect(rownames(b6_expr_top5), rownames(cast_expr)),,drop=F])

power_both <- lapply(b6_and_cast, SFT_fit, cor_method="pearson")
net_both <- lapply(seq_along(power_both), function(n) 
  exp2gcn(b6_and_cast[[n]], SFTpower = power_both[[n]]$power, 
          cor_method = "pearson")
  )
...
tibble( gene_id = intersect(rownames(b6_expr_top5), rownames(cast_expr))) |> 
...
b6_cast_coldata <- colData(b6_cast_dds_all) |> 
  as_tibble() |> 
  select(unique_sampleid, sex,ind, rep, AP_domain, Gest_stage, LB_stage, Palate_ori, strain) %>%
  mutate( AP_domain = factor(AP_domain, levels = c("PM","MAX","POST")) ) %>%
  column_to_rownames("unique_sampleid")
...
```{r}
#| label: module_genes_table_b6_cast
#| message: false
#| warning: false


net_b6_cast$genes_and_modules |> 
  left_join( all_genes_palate |> 
               select( Genes = gene_id, symbol, chromosome)
            ) |> 
  group_by(Modules) |> 
...
plot_eigengene_by_module_genes <- function(mod_name, module_eigengenes_long, expr, coldata,module_genes){
    ...
    gene_plot <- pheatmap(mat = mat,
         ...
         clustering_distance_cols = "euclidean",
         color = viridisLite::plasma(512, begin = 0, end = 1,direction = -1 ),
         annotation_col = coldata[,c("AP_domain","LB_stage","strain","Palate_ori")],
         annotation_colors = list( 
...
#| fig-width: 10
#| fig-height: 6
corrplot(corr = cormat[,c("B6","CAST","M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F],
         p.mat = as.matrix(pmat[,c("B6","CAST","M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]),
         sig.level = 0.05,
         insig = "label_sig", 
         pch.cex = 1.3, 
         pch.col = "red",
...
#| warning: false
#| message: false
cormat[,c("B6","CAST","M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F]|> 
  as_tibble(rownames = "module") |> 
  pivot_longer(2:9, names_to = "effect", values_to = "Correlation") |> 
  full_join(
    pmat[,c("B6","CAST","M","F","PM","MAX","POST","LB_stage","Left","Right"),drop=F] |> 
      as_tibble(rownames = "module") |> 
      pivot_longer(2:9, names_to = "effect", values_to = "Adjusted p-value")
  ) |> 
  filter(`Adjusted p-value` < 0.05) |> 
