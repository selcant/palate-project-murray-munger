...
## Genetic mapping with PC1

```{r}

pc1_rankz <- apply(pca$x[,"PC1", drop=FALSE],2, rankZ)

pc1_qtl <- scan1(pheno = pc1_rankz,
                      genoprobs = genoprobs,
                      kinship = kinship_loco,
                      addcovar = covar[,"sex",drop=F]
)
...
pc1_perm <- scan1perm(genoprobs = genoprobs, 
                     pheno = pc1_rankz,
                     addcovar = covar[,"sex",drop = FALSE], 
                     n_perm = 1000
                  )
...
thres <- summary(pc1_perm, alpha = c(0.05, 0.1)) %>% 
  as_tibble( rownames = "alpha")

pc1_peaks <- find_peaks(pc1_qtl,
                        threshold = 6,#thres$PC1[2], 
                        pmap)
...
pc1_scan_df <- pc1_qtl %>% 
  as_tibble( rownames = "marker") %>% 
  left_join(map_dat2) 
pc1_scan_df$pos_bp_offset <- pc1_scan_df$pos_bp+chrom_lens_offset[pc1_scan_df$chr]
...
ggplot()+
  geom_rect( data = chrom_segments, 
             ...
             inherit.aes = FALSE, alpha = 0.2, show.legend = FALSE)+
  scale_fill_manual(values = c("dark gray","white"))+
  geom_line(data = pc1_scan_df ,
            aes( x = pos_bp_offset, y = PC1),
            size = 1, 
            col =strain_colors[[3]],
             inherit.aes = FALSE )+
  theme_pubclean(base_size = 16)+
  ...
  ylab("LOD")+
  ylim(0,8)
  # geom_hline( yintercept = thres$PC1[1], col = "red", lty = 2)+
  # geom_hline( yintercept = thres$PC1[2], col = "red", lty = 3)
...
pc1_peak_chr13 <- find_peaks( pc1_qtl, threshold = 6, pmap) |> filter(chr == 13)

pc1_qtl_eff <- scan1blup(pheno = pc1_rankz,
                      genoprobs = genoprobs[,"13"],
                      kinship = kinship_loco[["13"]],
                  addcovar = covar[,"sex",drop = FALSE],
)

marker   <- map_dat2 %>% filter(pos_bp == pc1_peak_chr13$pos)
geno     <- pull_genoprobpos(genoprobs,marker$marker)
pc1_peak_chr13_eff_sum <- (pc1_qtl_eff[marker$marker,,drop=F])




pc1_peak_chr13_lod_plot <- pc1_scan_df %>% 
  filter( chr == 13) %>% 
  ggplot()+
  aes( 
    x= pos_bp/1e06,
  ...
  xlab(paste0("Chr 13 location (Mbp)"))+
  ylab( "LOD score")+
  annotate( "text", x= 6+filter( bands.rna,chr==13)$end[1]/1e06, y = 6, label ="eQTL \n Hotspot", size =5)+
  ylim(0,8)+
  xlim(60,120)
  
pc1_peak_chr13_eff_plot <- pc1_peak_chr13_eff_sum[,LETTERS[1:8]] %>% 
  as_tibble(rownames = "effect") %>% 
  mutate( type = "PC1") %>% 
  mutate( effect = case_when( effect == "A" ~ "AJ",
                              effect == "B" ~ "B6",
...
pc1_peak13_plot<- ggarrange(pc1_peak_chr13_lod_plot, pc1_peak_chr13_eff_plot, widths = c(1, 0.5))
pc1_peak13_plot
...
plot_coefCC( pc1_qtl_eff, pmap, scan1_output = pc1_qtl, xlim= c(1e08, 1.1e08), main = "PC1 QTL of the full data")
abline( v =pc1_peak_chr13$pos, col = "black", lwd = 2, lty = 2)
abline( v =chr13_pc1_peak$pos, col = "red", lwd = 2, lty = 2)
abline( v = filter( bands.rna,chr==13)$start[1], col = "blue", lwd = 2)
abline( v =filter( bands.rna,chr==13)$end[1], col = "blue", lwd = 2)
...
pc1_chr13_peak_as <- scan1snps(genoprobs = genoprobs,
                     map = pmap_Mbp,
                     pheno = pc1_rankz,
                     kinship = kinship_loco[[pc1_peak_chr13$chr]],
                     addcovar = covar[,"sex",drop=F],
                     chr = as.character(pc1_peak_chr13$chr),
                     query_func = query_variants,
                     start = pc1_peak_chr13$pos/1e06 - 5,
                     end = pc1_peak_chr13$pos/1e06 + 5,
                     keep_all_snps = T
                     )
genes <- query_genes( pc1_peak_chr13$chr,
                      pc1_peak_chr13$pos/1e06 - 5,
                      pc1_peak_chr13$pos/1e06 + 5) %>%
  distinct()
...
plot(pc1_chr13_peak_as$lod,
     pc1_chr13_peak_as$snpinfo,
     drop_hilit = 1,
     genes = genes,
     xlim=c(100,108))
...
SNPs specific to NZO are all in *Mast4*. According to MGI, mice homozygous for an ENU-induced allele exhibit malocclusion.

```{r pc1_chr13_peak_as_table}
#| warning: false
#| message: false
pc1_chr13_peak_as$lod |> 
  as.data.frame( ) |> 
  rownames_to_column("snp_id") |> 
  left_join( pc1_chr13_peak_as$snpinfo) |> 
  filter( PC1 > 4) |> 
  select( LOD = PC1,
          snp_id ,
          pos ,
...
```{r pc1_chr13_meds_table}
#| warning: false
#| message: false
pc1_med %>% 
  mutate( qtl_pos = pc1_peak_chr13$pos, qtl_chr = pc1_peak_chr13$chr, qtl_lod = pc1_peak_chr13$lod) %>% 
  mutate( scaled_LOD = scale(LOD)) %>% 
  filter( abs(qtl_pos-midpoint) <= 10e6 &   # filter within 10Mb
            qtl_chr == chr
  ) %>% 
...
### Closer look at a subset of PC1 drivers

There are `r nrow(pc1_drivers)` PC1 driver genes where `r length(intersect(pc1_drivers$id, chr13.eQTL1$gene_id))` also are targets of the Chr 13 eQTL hotspot. What are those genes?


```{r}

chr13.eQTL1 |> 
  filter( gene_id %in% pc1_drivers$id) |> 
  select(symbol, peak_chr, lod, peak_pos = interp_bp_peak) |>
  mutate_if(is.numeric, round, 2) |> 
  create_dt()

```
How does the eQTL allele effects look like?
...
pc1_drivers_overlap_chr13 <-intersect(pc1_drivers$id, chr13.eQTL1$gene_id)


pc1_drivers_overlap_chr13_mat <- chr13.eQTL1 %>%
  filter( gene_id %in% pc1_drivers_overlap_chr13) |>
  filter(!is.na(A)) %>%
  select(LETTERS[1:8],"symbol") %>%
  distinct() %>%
  column_to_rownames("symbol") %>%
...
annotation_row <- data.frame(strain=c("AJ","B6","129","NOD","NZO","CAST","PWK","WSB"))
rownames(annotation_row) <- rownames(pc1_drivers_overlap_chr13_mat)

annot.colors <- list(strain = founder_colors)

# svg(filename = here("figures","chr1_hotspot_allele_eff.svg"),width = 800, height = 600)
pheatmap(pc1_drivers_overlap_chr13_mat,
         cluster_rows=T,
         show_rownames=FALSE,
         cluster_cols=T,
         show_colnames = FALSE,
