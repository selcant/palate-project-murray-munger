...
b6_domain_eff_max_pm <- results(b6_dds_coll, contrast = list("AP_domainMAX")) 
b6_domain_eff_post_pm <- results(b6_dds_coll, contrast = list("AP_domainPOST")) 
b6_domain_eff_post_max <- results(b6_dds_coll, contrast = list("AP_domainPOST","AP_domainMAX")) 
...
b6_domain_eff_gene_counts <- counts(b6_dds_coll, normalized=TRUE)[unique(b6_domain_eff_merged$gene_id),] %>% 
  as_tibble( rownames = "gene_id") %>% 
...
#### Genes uniquely differentially expressed in the POST domain
...
g.b6_domain_all_up <- gost( query = unique( (b6_domain_all_diff_genes %>% 
                                                 filter( Comparison =="POST > MAX > PM"))$symbol),
                           organism = "mmusculus",
                           domain_scope = "custom",
                           custom_bg = all_genes_palate$symbol,
                           evcodes = TRUE
                           )
g.b6_domain_all_up$result <- g.b6_domain_all_up$result %>% filter(term_size < 660)
...
b6_stage_s135_vs_125 <- results(b6_dds_coll, contrast = list(c("Gest_stageE13.5")))
b6_stage_s145_vs_125 <- results(b6_dds_coll, contrast = list(c("Gest_stageE14.5")))
b6_stage_s155_vs_125 <- results(b6_dds_coll, contrast = list(c("Gest_stageE15.5")))
b6_stage_s145_vs_135 <- results(b6_dds_coll, contrast = list(c("Gest_stageE14.5","Gest_stageE13.5")))
b6_stage_s155_vs_135 <- results(b6_dds_coll, contrast = list(c("Gest_stageE15.5","Gest_stageE14.5")))
b6_stage_s155_vs_145 <- results(b6_dds_coll, contrast = list(c("Gest_stageE15.5","Gest_stageE14.5")))
...
# I am changing k to 8 and kxdist to 1 (no clustering based on x distance) only x-y patterns.
...
```{r b6_stage_eff_curve_cluster_reps}
#| warning: false
#| message: false
#| fig-width: 16
#| fig-height: 10

b6_stage_eff_cluster_data_mean <- b6_stage_eff_cluster_data %>% 
  group_by(time,cluster) %>% 
  summarize(count = median(count)) 

b6_stage_eff_cluster_data_mean %>% 
  ggplot()+
  aes(x = time, y = count, col = as.factor(cluster))+
  geom_point()+
  geom_line()+
  facet_wrap(~cluster,scales = "free", nrow = 2)+
  theme_pubclean()+
  scale_x_continuous(name = "Gestational stage", breaks = c(12.5, 13.5, 14.5, 15.5))+
  ylab("Median gene expression by time point")

```


Looking closer at individual clusters:

#### Cluster 1

```{r b6_stage_eff_cluster1}

b6_stage_eff_cluster_data %>% 
  filter(cluster == 1) 
...
I am using likelihood ratio test (LRT) to compare a linear model with and without an interaction between strain and gestational stage. This test allows us to identify genes that change differently between two strains across gestational stages. There are `r formatC(nrow(strain_by_gest_lrt_merged %>% select(gene_id) %>% distinct()), big.mark =",")` genes that show strain effects at one or more time points after E12.5. I am plotting some examples below to visualize and also doing an ORA with this gene list.
