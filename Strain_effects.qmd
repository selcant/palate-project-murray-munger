...
This first notebook includes:
  ...
  -   Gene set variation analysis using filtered and normalized counts to identify biological processes and mammalian phenotypes that vary between B6 and CAST, across domains and over developmental time.
  -   Weighted gene coexpression network analysis using 
...
Visualize driver genes in a 3D plot:
```{r}
#| label: pc5_3d_loadings
...
loadings <- pca$rotation[all_genes_palate_filtered$gene_id,,drop=F]
rownames(loadings) <- all_genes_palate_filtered$symbol
 # Extracting the loadings for PC5
loadings_pc5 <- loadings[, 5]

# Identifying the top contributors
top_contributors <- order(abs(loadings_pc5), decreasing = TRUE)[1:10] 
# Adding vectors for loadings
for (i in 1:nrow(loadings)) {
  p <- add_trace(p, x = c(0, loadings[i, 1]), y = c(0, loadings[i, 2]), z = c(0, loadings[i, 5]), 
                 mode = 'lines', line = list(color = 'red'), name = rownames(loadings)[i])
}



# Adding labels for the top contributors
for (i in top_contributors) {
  p <- add_annotations(p, x = loadings[i, 1], y = loadings[i, 2], z = loadings[i, 3], 
                       text = rownames(loadings)[i], showarrow = TRUE, arrowhead = 2)
}
...
Running GSVA with only the B6 and CAST samples, filtering out DO. 
```{r run_gsva}
#| warning: false
#| message: false
#| results: hide
...
expr.upd <- assay( dds_all_norm)[all_genes_palate_filtered$gene_id, !startsWith(colnames(assay( dds_all_norm)[all_genes_palate_filtered$gene_id,,drop=F]),"DO"),drop=F]
rownames(expr.upd) <- all_genes_palate_filtered$symbol
...
Below is a table of Mammalian Phenotypes that show significant differences between strain, domain or over developmental time (adjusted p-value \< 0.01). There are too many go terms to display, instead I made the plots below to summarize the results for each effect.
...
Let's simplify the GO terms that show significant differences between LB stage bins (adjusted p-value \< 0.01):
...
Let's compare modules by calculating the # of genes that overlap between modules and the Jaccard Index. The Jaccard Index is a way to measure how similar two sets are by comparing the number of items they have in common to the total number of unique items in both sets.
...
final_expr_all <- assay(dds_all_norm)[intersect(rownames(b6_expr), rownames(cast_expr)),,drop=F] |> 
...
save(MOFAobject_untrained, file = here("_data/mefisto_trial_untrained_model_justStrain.Rdata"))

# running on the interactive cluster using the docker container
outfile <- ("/projects/munger-lab/projects/palate-project-murray-munger/analysis/_data/mefisto_trial_just_strain_5factors.hdf5")
...
plot_factors(MOFAobject, 1:5, color_by = "time", shape_by = "domain") 
...
correlate_factors_with_covariates(object = MOFAobject,
                                  covariates = sample_metadata)
