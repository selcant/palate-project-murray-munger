...
b6_domain_by_stage_post_pm <- results( b6_dds_comp, contrast = list(c("AP_domainPOST","AP_domainPOST.LB_stage"))) %>%
  ...
  filter( padj < 0.05 & !is.na(padj) & abs(log2FoldChange) > .5)  %>%
  left_join( all_genes_palate)
b6_domain_by_stage_post_max <- results( b6_dds_comp, contrast = list(c("AP_domainPOST","AP_domainPOST.LB_stage"),c("AP_domainMAX","AP_domainMAX.LB_stage"))) %>%
  as_tibble( rownames = "gene_id") %>%
  filter( padj < 0.05 & !is.na(padj) & abs(log2FoldChange) > .5)  %>%
  left_join( all_genes_palate)
...
### Genes differentially expressed between B6 and CAST in each domain (with strain by time interaction)
...
# get genes
strain_by_time_pm_genes <- results(strain_dds_int_pm, contrast = list(c("strainCAST","strainCAST.LB_stage")) ) %>%
  as_tibble( rownames = "gene_id") %>%
...
### Genes differentially expressed over time in each domain (with strain by time interaction)
