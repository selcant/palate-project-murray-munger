...
b6_datExpr <- t(assay(b6_dds_all_norm)[ B6rv_wpn > B6q90_wpn, ])
...
module_stability(b6_expr_final, net_b6, nRuns = 5)
...
Using `r formatC(nrow(b6_expr_final),big.mark=",")` genes and `r formatC(ncol(b6_expr_final),big.mark=",")` samples. For gene coexpression network inference I am using the `exp2gcn` function in BioNero with the following options:
  ...
  -   `SFTpower = 16`: I chose 16 as the thresholding power since it is the power where the scale free topology fit R2 value reaches 0.79. 
...
Get sub-networks within modules and visualize:

```{r}
#| label: get_subnetworks
#| results: hide
#| warning: false
#| message: false
...
Using `r formatC(nrow(cast_expr_final),big.mark=",")` genes and `r formatC(ncol(cast_expr_final),big.mark=",")` samples. 
  ...
  -   `SFTpower = 13`: I chose 16 as the thresholding power since it is the power where the scale free topology fit R2 value reaches 0.79. 
...
preserved_modules_b6_cast <- pres$p.values |> 
  as_tibble( rownames = "Module") |> 
  #filter(if_all(everything(),  ~  . < 0.05))
  filter( coherence < 0.05)
...
plot_module_overlap <- function(comparison_results, p_threshold = 0.05, highlight_modules = NULL) {
  ...
  plot_data <- comparison_results %>%
    ...
    mutate(
      ...
      highlight_color = ifelse(module1 %in% highlight_modules, "red", "black")
    )
  ...
  ggplot(plot_data, aes(x = module1, y = module2, fill = jaccard)) +
    ...
    theme_minimal() +
    theme(
      axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1, color = plot_data$highlight_color),
...
# Create the plot
plot_module_overlap(comparison_results, p_threshold = 0.01, highlight_modules = preserved_modules_b6_cast$Module)+
    labs(
      x = "B6", 
      y = "CAST"
      )
...
Using `r formatC(nrow(do_expr_final),big.mark=",")` genes and `r formatC(ncol(do_expr_final),big.mark=",")` samples. 
  ...
  -   `SFTpower = 7`: I chose 16 as the thresholding power since it is the power where the scale free topology fit R2 value reaches 0.79. 
...
```{r}
#| label: do_b6_cast_mod_preservation
#| warning: false
#| message: false
#| eval: false
...
pres_do <- module_preservation( b6_and_do,
                            ref_net = net_b6, 
                            test_net = net_do, 
                            algorithm = "netrep",
                            nThreads = 40)

preserved_modules_b6_do <- pres_do$p.values |> 
  as_tibble( rownames = "Module") |> 
  #filter(if_all(everything(),  ~  . < 0.05))
  filter( coherence < 0.05)
