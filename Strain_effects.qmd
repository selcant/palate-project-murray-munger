...
for( mod_col in unique(module_genes$Modules)){
  ...
  cat("\n####",mod_col, "module","\n")
  p <- pheatmap(mat = log1p(t(b6_datExpr[rownames(b6_coldata_ordered),mod_genes$Genes, drop = FALSE])),
         show_rownames = F,
         show_colnames = F, 
         cluster_rows = T,
         cluster_cols = F,
...
module_eigengenes_long <-  module_eigenges |> 
  pivot_longer( MEred:MEgrey, names_to = "module",values_to = "value") |> 
  mutate( mod_col = factor(gsub("ME","",module) ))
...
for( mod_col in unique(module_genes$Modules)){
  ...
  cat("\n####",mod_col, "module","\n")
  p <- pheatmap(mat = log1p(t(cast_datExpr[rownames(cast_coldata_ordered),mod_genes$Genes, drop = FALSE])),
         show_rownames = F,
         show_colnames = F, 
         cluster_rows = T,
         cluster_cols = F,
...
module_eigengenes_long <-  module_eigenges |> 
  pivot_longer( MElightsteelblue1:MEgrey, names_to = "module",values_to = "value") |> 
  mutate( mod_col = factor(gsub("ME","",module) ))
...
module_eigengenes_long <-  module_eigenges |> 
  pivot_longer( MEblack:MEgrey, names_to = "module",values_to = "value") |> 
  mutate( mod_col = factor(gsub("ME","",module) ))
