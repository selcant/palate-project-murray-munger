...
#### Genes only differentially expressed in the PM domain
...
#### Genes uniquely differentially expressed in the MAX domain
...
b6_domain_all_diff_genes <-  b6_domain_eff_merged %>% 
  ...
  filter(
   `MAX vs PM_padj` < 0.05 &  `POST vs PM_padj` < 0.05 & `POST vs MAX_padj` < 0.05 & 
    abs(`MAX vs PM_log2FoldChange`) > .5  & abs(`POST vs PM_log2FoldChange`) > .5 & abs(`POST vs MAX_log2FoldChange`) > .5) %>% 
    mutate( Comparison = ifelse( (`MAX vs PM_log2FoldChange`) > 0  & 
          ...
          ifelse((`MAX vs PM_log2FoldChange`) < 0  & 
            (`POST vs PM_log2FoldChange`) < 0 & 
            (`POST vs MAX_log2FoldChange`) < 0 , "POST < MAX < PM" , NA))
  ) %>% 
  left_join(all_genes_palate)

# 
# b6_domain_all_up_genes <- b6_domain_all_diff_genes %>% 
#   filter( (`MAX vs PM_log2FoldChange`) > 0  & 
#             (`POST vs PM_log2FoldChange`) > 0 & 
#             (`POST vs MAX_log2FoldChange`) > 0)
# b6_domain_all_down_genes <- b6_domain_all_diff_genes %>% 
#   filter( (`MAX vs PM_log2FoldChange`) < 0  & 
#             (`POST vs PM_log2FoldChange`) < 0 & 
#             (`POST vs MAX_log2FoldChange`) < 0)

# filter(b6_domain_all_diff_genes , is.na(Comparison)) -- what do you wanna do with these?
