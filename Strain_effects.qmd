...
# Am I getting any weird results in stage by domain?
# That one makes sense where I am getting the stage effects for each domain when I add the interaction term. 

a <- b6_stage_by_domain_merged %>% 
  select(gene_id, symbol, baseMean, log2FoldChange, padj,Comparison) %>% 
    mutate( Comparison = case_when( log2FoldChange >0 ~ "Upregulated over time",
                                  log2FoldChange <0 ~"Downregulated over time")
          ) 
...
c <- inner_join( select(a, symbol,baseMean), select(b, symbol, baseMean)) %>% distinct()
d <- inner_join( select(a, symbol, baseMean, Comparison), select(b, symbol, baseMean, Comparison)) %>% distinct()

assertthat::are_equal(select(c,symbol,baseMean) %>% arrange(baseMean), select(d,symbol,baseMean) %>% arrange(baseMean))
# Yes, we are getting the same directionality! So no problems here.

# I am getting weird results from domain by stage. 
# Note that it is not clear to me what comparison I am making here though.
# I am using the interaction term for each comparison BUT I don't really know
# what the actual comparison is with/without the interaction term since there is no 
# reference level for a continuous variable (acc to M.Love).
b6_domain_eff_genes_merged <- b6_domain_eff_max_pm %>%
  mutate( Comparison = "MAX vs PM") %>%
  rbind(
    b6_domain_eff_post_pm %>%
...
# What is going on with sox2?
sox2 <- all_genes_palate %>% filter(symbol =="Sox2")

b6_domain_by_stage_merged %>% 
  filter(gene_id == sox2$gene_id) %>% 
  select(gene_id, symbol, baseMean, log2FoldChange, padj,Comparison)
...
ggplot(filter(b6_gene_counts, gene_id == sox2$gene_id))+aes(x=LB_stage, y = count, col = AP_domain)+geom_point()+geom_line()+theme_pubclean()+scale_color_manual(values = domain_colors)

ggplot(filter(b6_gene_counts, gene_id == sox2$gene_id))+aes(x=AP_domain, y = count, col = AP_domain)+geom_point()+theme_pubclean()+scale_color_manual(values = domain_colors)

ggplot(filter(b6_gene_counts, gene_id == "ENSMUSG00000018411"))+aes(x=LB_stage, y = count, col = AP_domain)+geom_point()+geom_line()+theme_pubclean()+scale_color_manual(values = domain_colors)

ggplot(filter(b6_gene_counts, gene_id == "ENSMUSG00000018411"))+aes(x=AP_domain, y = count, col = AP_domain)+geom_point()+theme_pubclean()+scale_color_manual(values = domain_colors)


# are there any other genes like Sox2? Looks like there are 149 of them! 
b6_domain_by_stage_merged %>% 
  #filter(gene_id == sox2$gene_id) %>% 
  select(gene_id, symbol, baseMean, log2FoldChange, padj,Comparison) %>% 
  mutate( Comparison = paste(Comparison, "int model")) %>% 
  rbind(
    b6_domain_eff_genes_merged %>% 
      #filter(gene_id == sox2$gene_id) %>% 
      select(gene_id, symbol, baseMean, log2FoldChange, padj,Comparison)
  ...
  filter(
   (`MAX vs PM int model_log2FoldChange`*`MAX vs PM_log2FoldChange` < 0) |
   (`POST vs PM int model_log2FoldChange`*`POST vs PM_log2FoldChange` < 0) |
   (`POST vs MAX int model_log2FoldChange`*`POST vs MAX_log2FoldChange` < 0)
  ) %>% 
  select(gene_id, `MAX vs PM int model_log2FoldChange`,`MAX vs PM_log2FoldChange`,
         `POST vs PM int model_log2FoldChange`,`POST vs PM_log2FoldChange`,
         `POST vs MAX int model_log2FoldChange`,`POST vs MAX_log2FoldChange`) %>% view()
...
In addition, there are `r formatC( b6_domain_by_stage_merged %>% select(gene_id) %>% distinct() %>% nrow(), big.mark = ",")` genes that are differentially expressed between domains over time (domain by stage effect) and `r formatC( b6_stage_by_domain_merged %>% select(gene_id) %>% distinct() %>% nrow(), big.mark = ",")` genes that change differently between domains over time (stage by domain effect).
...
b6_dds_comp_norm_wclusters %>% 
  ...
  left_join( all_genes_palate) %>% 
  select( gene_id, symbol, log2FoldChange, padj) %>%
  distinct() %>% 
...
Alternative grouping by clustering:
...
#   as_tibble(rownames = "genes") %>%
#   pivot_longer(cols = 2:ncol(.), names_to = "sample", values_to= "expression") %>%
#   right_join(distinct(lbstage_clusters$df[,c("genes", "cluster")]),
#                by = "genes") %>%
#   left_join( as_tibble(colData(b6_dds_comp), rownames = "sample") )
...
### Genes with different trends over time across domains (with time by domain interaction)
...
### Genes differentially expressed between B6 and CAST in each domain over time (with strain by time interaction)
...
time_by_strain_pm_genes_b6 <- results(strain_dds_int_post, contrast = list(c("LB_stage")) ) %>%
  ...
  mutate(
    Comparison = ifelse( log2FoldChange >0, "Upregulated over time in B6", "Downregulated over time in B6")
  ) %>%
  left_join(all_genes_palate)
time_by_strain_pm_genes_cast <- results(strain_dds_int_post, contrast = list(c("LB_stage","strainCAST.LB_stage")) ) %>%
  ...
  mutate(
    Comparison = ifelse( log2FoldChange >0, "Upregulated over time in CAST", "Downregulated over time in CAST")
  ) %>%
  left_join(all_genes_palate)
...
time_by_strain_pm_genes_b6 %>% 
  ...
  select(gene_id, Comparison) %>%
  mutate( Comparison = case_when(
    Comparison =="Upregulated over time in B6, Upregulated over time in CAST"~"Up in both",
    Comparison =="Downregulated over time in B6, Downregulated over time in CAST"~"Down in both",
    Comparison =="Upregulated over time in B6"~"Up in B6",
    Comparison =="Downregulated over time in B6"~"Down in B6",
    Comparison =="Upregulated over time in CAST"~"Up in CAST",
    Comparison =="Downregulated over time in CAST"~"Down in CAST",
    Comparison =="Upregulated over time in B6, Downregulated over time in CAST"~"Up in B6, Down in CAST",
  )) %>% 
