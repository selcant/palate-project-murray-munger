...
This notebook is analyzing the palate gene expression from different genetic backgrounds, AP domains obtained at various gestational stages. The first section uses the normalized gene expression counts from B6, CAST and DO mice to look at sources of variation. The second section focuses on B6 and CAST gene expression to specifically identify differentially expressed genes across domains, gestational stages and strain backgrounds. Finally, the third section performs weighted gene co-expression network analysis using the gene expression data from B6 and CAST strains. The genetic characterization of gene expression and developmental phenotypes using the DO strains can be found in `Palate_qtl_mapping.html` notebook.
...
#      ) %>% 
#   openxlsx::write.xlsx(., file = here("_data","Gene_counts_for_Ian_09132023.xlsx"))
...
save( expr_mat,
      ...
      all_genes_palate,
      note,
      file = here("_data","Gene_counts_for_Ian_09132023.RData")
     )

downloadthis::download_file(
  path = system.file(here("_data","Gene_counts_for_Ian_09132023.xlsx"),
...
In the final data set we have `r formatC(nrow(expr_mat), big.mark=",")` genes expressed across `r nrow(sample_annot)` samples. The first and second principal components (PCs) group samples by developmental time and strain background respectively. Together the first two PCs explain 35% of the variation in gene expression. Drivers of PC1 include genes involved in extracellular matrix and intermediate filament organization, collagen biosynthesis as well as processes related to muscle and bone development. In addition, PC1 and PC2 drivers are overrepresented for genes that contain binding motifs for transcription factor (TF) SRF. PC4 groups samples by AP domain irrespective of their strain background or developmental stage. PC4 driver genes are involved in nervous system, muscle and bone development related processes in addition to BMP and WNT signaling and are overrepresented for targets of transcription factor NRSF. Sex of the animals show up at PC7 and PC8 where together they are able to group female and male samples distinctly and explain only 4% of the variation in gene expression.
...
In the tabs below you can find the detailed analyses. Briefly, I analyzed the B6 and CAST data separately to identify genes differentially expressed in each background across AP domains, over developmental time and change differently over time across domains. Next, I analyzed the B6 and CAST data together while separating the data by AP domains. For the individual strain analysis, I am focusing on the data from only the B6 or CAST animals that includes males and females, and contain the three AP domains (PM, MAX and POST) obtained at various developmental stages (LB_stage). I merge the technical replicates due to resequencing of several samples using `collapseReplicates()` function from DESeq2. We expect to have genes that show significant differences between AP domains, over developmental time and genes with different trends over time across the three tissues. In order to identify differentially expressed genes between AP domains and over time I obtain the main effects for AP_domain and LB_stage while using a nesting for the AP domain since they are obtained from the same animal (\~ AP_domain + LB_stage + AP_domain:ind). For identifying genes with different trends over time across the three AP domains I use the likelihood ratio test (LRT) to compare the initial simple model to the one that includes an interaction term between AP domain and limb bud stage (\~ AP_domain + LB_stage + AP_domain:ind + AP_domain:LB_stage). The LRT will not return genes where the trend over time is not different across domains, even though the genes may be differentially expressed between domains at a particular time point. The significant DE genes will be those that show differences in trends over time in at least two AP domains. Once I have this DE gene list, I then use the full model to get the effects of developmental time in all three domains. *Note that* we need to be very careful when interpreting the main effects with a linear model that contains an interaction term, they do not represent the overall effect of the predictor (e.g. LB_stage) since the predictors involved in an interaction do not have a single overall effect anymore! For example the main effect for LB_stage will represent the effect when AP_domain is at the reference level which is the PM domain in our data set. 


<!-- **Summary of comparing the separate B6 and CAST analysis:** -->

<!-- -   Genes differentially expressed between AP domains: -->

<!--     -   PM: 79/142/173 shared/b6/cast up, 126/216/322 shared/b6/cast down. -->
<!--         Up in B6 & CAST: epithelial to mesenchymal transition. -->
<!--         Down in B6 & CAST: behavior, synaptic signaling. -->
<!--         CAST has many more results than B6! -->
<!--         Up in B6: Wnt signaling pathway, Signaling by GPCR Up in CAST: limb development, anterior/posterior pattern specification, axonogenesis, ossification, skeletal system development, Signaling pathways regulating pluripotency of stem cells. -->
<!--         Down in CAST: brain development, regulation of neuron differentiation, axonogenesis, regulation of nervous system development, cAMP signaling pathway, NRSF. -->

<!--     -   MAX: 22/63/81 shared/b6/cast up, 15/28/45 shared/b6/cast down. -->
<!--         No common categories in ORA results. -->
<!--         Up in B6: brain development. -->
<!--         Down in B6: nephron epithelium development. -->
<!--         Up in CAST:embryonic organ development, mesenchymal cell proliferation, muscle tissue development. -->
<!--         Down in CAST: connective tissue development. -->

<!--     -   POST: 126/243/237 shared/b6/cast up, 116/246/209 shared/b6/cast down. -->
<!--         Down in both: Pax3-Lef1-Tle4 complex, embryonic organ development, epithelial cell differentiation, anterior/posterior pattern specification, embryonic limb morphogenesis, limb development, proximal/distal pattern formation, mesenchyme development, endocrine system development, reproductive system development, mesenchymal cell differentiation, skeletal system development, axon guidance, mesoderm development, epidermis development, forebrain generation of neurons, axonogenesis, gonad development, bone development, WNT ligand biogenesis and trafficking. -->
<!--         Nothing common in ORA results for up. -->
<!--         Up in B6: muscle structure development. -->
<!--         Down in B6: Signaling pathways regulating pluripotency of stem cells, Hippo signaling pathway. -->

<!--     -   pm\>max\>post: 10/22/21. -->
<!--         No pathways/processes in B6. -->
<!--         CAST: embryonic epithelial tube formation. -->

<!--     -   pm\<max\<post: 11/29/40. -->
<!--         No pathways/processes in B6 or CAST. -->

<!-- -   Genes differentially expressed over time: -->

<!--     -   Upregulated over time: 948/1387/1086 shared/b6/cast. -->
<!--         Similar results from ORA from both strains, there are so many I didn't try to summarize here though. -->
<!--         Junb, Fra2 shared. -->
<!--         CAST only: Smad, Fra1. -->

<!--     -   Downregulated over time: 51/132/70 shared/b6/cast. -->
<!--         Shared: SRF motif. -->
<!--         In B6: Irf4, Ahr, Parp, Pbx1 motifs also show up. -->

<!-- -   Genes differentially expressed over time between domains: -->

<!--     -   PM: 264/693/374 up & 31/103/57 down shared/b6/cast. -->
<!--         Up in B6 & CAST: muscle tissue development, Calcium signaling pathway, PI3K-Akt signaling pathway, Collagen biosynthesis and modifying enzymes Up in B6: G protein-coupled receptor signaling pathway, positive regulation of MAPK cascade, regulation of inflammatory response, ossification, biomineral tissue development Up in CAST: epidermal cell differentiation, keratinocyte differentiation Down in B6: mesenchyme development. -->
<!--         Down in CAST: nothing. -->

<!--     -   MAX: 280/674/495 up & 20/87/33 down shared/b6/cast. -->
<!--         Up in B6 & CAST: muscle organ development, synaptic signaling Up in B6: positive regulation of MAPK cascade, inflammatory response, ossification, Calcium signaling pathway, cAMP signaling pathway Up in CAST: odontogenesis, G protein-coupled receptor signaling pathway, NRSF, Down in B6: sensory organ development, embryonic morphogenesis, limb development , ear development. -->
<!--         Down in CAST: nothing. -->

<!--     -   POST: 242/628/434 up & 36/84/73 down shared/b6/cast. -->
<!--         Up in B6 & CAST: leukocyte migration, biomineral tissue development, muscle contraction, O-glycosylation of TSR domain-containing proteins Up in B6: synaptic signaling, G protein-coupled receptor signaling pathway, ossification, Calcium signaling pathway Up in CAST: heart contraction, , inflammmatory response. -->
<!--         Down in B6 & CAST: Rag1-Rag2 protein-DNA complex, Down in B6: embryonic morphogenesis, subpallium development Down in CAST: neural crest cell development, mesenchymal cell differentiation, retinoic acid metabolic process. -->

<!--     -   Trend: 73/259/144 up & 3/15/8 down shared/b6/cast. -->
<!--         Up in B6 & CAST: Muscle contraction, extracellular matrix organization. -->
<!--         Up in B6: leukocyte migration, bone mineralization, calcium ion homeostasis, ossification, O-glycosylation of TSR domain-containing proteins. -->
<!--         Up in CAST: nothing specific. -->
<!--         Down in B6 & CAST: Down in B6: nothing. -->
<!--         Down in CAST: nothing. -->
...
# let's run deseq2 with the simple and complete model. 
...
There are 1,852 genes differentially expressed across the three AP domains (adjusted p-value < 0.05 and 1.5 fold change). In addition, I am using likelihood ratio test (LRT) to compare the linear models with and without an interaction between AP domain and LB stage. This test allows us to identify genes that show a significant interaction effect meaning change in their expression will differ depending on the domain. There are 1,286 genes that change differently between domains over time. Below in each tab, I include the list of genes with details and overrepresentation analysis results.
...
b6_stage_eff_gene_counts <- counts(b6_dds_simple, normalized=TRUE)[unique(b6_stage_eff_merged$gene_id),] %>%
  as_tibble( rownames = "gene_id") %>%
  pivot_longer(2:65, names_to = "sample", values_to = "count") %>%
  left_join(
...
g.b6_stage_down <- gost( query = unique( (b6_stage_diff_genes %>%
...
There are `r formatC(nrow(b6_stage_eff_merged %>% filter( padj < 0.05 & !is.na(padj) & abs(log2FoldChange) > .5) %>% select(gene_id) %>% distinct()), big.mark = ",")` unique genes differentially expressed over time (adjusted p-value \< 0.05 and 1.5 fold change).
...
There are 1,879 unique genes differentially expressed across domains (adjusted p-value < 0.05 and 1.5 fold change). Then, I am using likelihood ratio test (LRT) to compare a linear model with and without an interaction between AP domain and LB stage. This test allows us to identify genes that change differently over developmental time across different domains. There are 887 genes that change differently between domains over time.
