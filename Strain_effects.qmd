...
-   Third PC groups samples by AP domain. 

-   Fourth PC separates the MAX AP domain from the other domains. 

-   PC8 groups samples based on sex. 
...
pc8_drivers <- pca$rotation[,"PC8", drop = FALSE] %>% 
  as_tibble( rownames = "id") %>% 
  left_join( all.genes_v84 %>%  
               select(id, symbol)
             ) %>% 
  filter( abs(PC8) >= quantile(abs(PC8), 0.95))
...
Table with the full list of PC8 driver genes:
...
Above I am plotting an example gene for each of the four categories separated in tabs below. On the left, there is a gene with a domain effect that doesn't show an interaction with stage.
...
Alternative grouping using clustering might be a better aproach for these genes. Let me know if any of these look interesting and I can follow up.
...
sft = pickSoftThreshold(datExpr, powerVector = powers, verbose = 0, blockSize = 20000)
...
# adj.mtx <- adjacency( log1p(datExpr), power=soft.power)
# TOM = TOMsimilarity(adj.mtx)
# diss.TOM = 1 - TOM
# gene.clust <- hclust(as.dist(diss.TOM), method="average")
# min.module.size <- 15
# modules <- cutreeDynamic(dendro=gene.clust, distM=diss.TOM, deepSplit=2, pamRespectsDendro=FALSE, minClusterSize=min.module.size)
# cols <- labels2colors(modules)
# # merging similar modules
# me.list <- moduleEigengenes(log1p(datExpr), colors=cols) 
...
# merge.thresh <- 0.25
# merge <- mergeCloseModules(log1p(datExpr), cols, cutHeight=merge.thresh, verbose=0)
...
# plotDendroAndColors(
#   gene.clust, cbind(cols, merged.cols), c("Dynamic Tree Cut", "Merged Dynamic"), xlab="", sub="",
...
As recommended by the authors, I am choosing the smallest power such that the scale-free topology correlation reaches 0.9. Then I generate the modules using the `blockwiseModules` function with the following parameters changed from the default values:
...
This leads to `r length(unique(net$colors))` modules in total which are characterized below. Of note, there are 13 modules that show significant correlation to strain, 15 modules that show significant correlation to A-P domain, 23 modules that show significant correlation to LB stage or gestational stage and 18 modules with significant correlation to palate closure state. 
