...
# Genes with a significant strain effect AND no interactions.
# The additive model LFC represents the average effect across
# domains and time, so interaction genes are excluded here.
# ===================================================================
...
main_patterns <- strain_effects_classified |>
  ...
  filter(main_strain_pattern != "No effect / filtered",
         !has_any_strain_interact) |>          # exclude interaction genes
...
# Note: genes_main_only removed — all genes in main_patterns are now interaction-free
...
# Uses passes_domain_filter (already excludes interaction genes).
# ===================================================================
...
# Uses passes_time_filter (already excludes interaction genes).
# ===================================================================
...
# Three-way interaction genes are excluded — they are analysed
# exclusively in Part 7.
# ===================================================================

domain_strain_patterns <- strain_effects_classified |>
  filter(any_strain_sig & strain_domain_int_sig,
         !threeway_int_sig) |>                  # exclude three-way genes
...
# ===================================================================
# PART 6: TIME × STRAIN INTERACTION PATTERNS
...
pattern_summary <- tibble(
  ...
  Pattern = c(
    # Main strain (interaction-free only)
  ...
  Description = c(
    "No interactions; marginal LFC > 1",
    "No interactions; 0.5 < marginal LFC ≤ 1",
    "No interactions; marginal LFC < -1",
    "No interactions; -1 ≤ marginal LFC < -0.5",
    ...
    "Domain×Strain sig, no three-way; PM has strongest |strain effect|",
    "Domain×Strain sig, no three-way; MAX has strongest |strain effect|",
    "Domain×Strain sig, no three-way; POST has strongest |strain effect|",
    "Domain×Strain sig, no three-way; strain effect direction varies across domains",
    "Domain×Strain sig, no three-way; strain effect range > 1.0 log2FC across domains",
    "Time×Strain sig, no three-way; positive slope (CAST increases faster)",
    "Time×Strain sig, no three-way; negative slope (B6 increases faster)",
...
gene_assignments <- all_classified_genes |>
  mutate(
    ...
    # Domain × Strain interaction (no three-way)
    ...
    # Time × Strain interaction (no three-way)
